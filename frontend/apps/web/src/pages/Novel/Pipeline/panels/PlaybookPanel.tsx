...
import {
  getPromptTemplates, updateRuleAtom, resetPromptTemplates, updateAgentSection, revertPromptEdit,
  type PromptTemplateView, type PromptSection, type PromptEditRecord, type RuleAtom,
...
const CATEGORY_META: Record<string, { label: string; color: string }> = {
  PROSE_CRAFT_PLAYBOOK: { label: '文笔技法', color: 'text-violet-500' },
  WRITING_SOUL_PLAYBOOK: { label: '写作灵魂', color: 'text-orange-500' },
  CONTINUITY_BASELINE_PLAYBOOK: { label: '连续性底线', color: 'text-blue-500' },
  THREAD_AWARENESS_PLAYBOOK: { label: '伏线意识', color: 'text-teal-500' },
  CHARACTER_ARC_PLAYBOOK: { label: '角色弧线', color: 'text-amber-500' },
  EDITOR_DISCIPLINE_PLAYBOOK: { label: '编辑纪律', color: 'text-rose-500' },
  REVIEWER_RUBRIC_PLAYBOOK: { label: '评审标尺', color: 'text-emerald-500' },
};
const AGENT_LABELS: Record<string, string> = {
  'creative-writer': '写手', 'scene-stitcher': '缝合', reviewer: '审阅', editor: '编辑',
  intent: '策划', 'scene-planner': '场景', 'arc-director': '导演', 'hook-crafter': '钩子',
};
const ALL_CATEGORY_KEYS = Object.keys(CATEGORY_META);

function RuleAtomCard({ atom, bookId, onUpdated }: { atom: RuleAtom; bookId: string; onUpdated: () => void }) {
  const [open, setOpen] = useState(false);
  const [content, setContent] = useState(atom.content);
  const [title, setTitle] = useState(atom.title);
  const [priority, setPriority] = useState(atom.priority);
  const [saving, setSaving] = useState(false);
  const changed = content !== atom.content || title !== atom.title || priority !== atom.priority;
  const handleSave = async () => {
    setSaving(true);
    try { await updateRuleAtom(bookId, atom.id, { title, content, priority }); onUpdated(); } finally { setSaving(false); }
  };
  const handleToggle = async (v: boolean) => {
    try { await updateRuleAtom(bookId, atom.id, { isEnabled: v }); onUpdated(); } catch {}
  };
  return (
    <div className={cn('rounded-lg border overflow-hidden transition-all', open ? 'border-primary/50 shadow-sm' : 'hover:border-muted-foreground/30')}>
      <div className="flex items-center gap-2 px-3 py-2 cursor-pointer" onClick={() => setOpen(!open)}>
        <Badge variant="outline" className="text-[9px] px-1 py-0 font-mono shrink-0">P:{atom.priority}</Badge>
        <span className={cn('text-[11px] font-medium flex-1 truncate', !atom.isEnabled && 'line-through opacity-50')}>{atom.title}</span>
        <div className="flex items-center gap-0.5 shrink-0">
          {atom.targetAgents.slice(0, 3).map((a) => <span key={a} className="text-[9px] px-1 py-0.5 rounded bg-muted text-muted-foreground">{AGENT_LABELS[a] ?? a}</span>)}
        </div>
        <Switch checked={atom.isEnabled} onCheckedChange={handleToggle} className="shrink-0 scale-75" onClick={(e) => e.stopPropagation()} />
      ...
      {open && (
        <div className="px-3 pb-3 space-y-2 border-t bg-background/50">
          <div className="flex gap-2 mt-2 items-center">
            <Input value={title} onChange={(e) => setTitle(e.target.value)} className="h-7 text-[11px] flex-1" placeholder="规则标题" />
            <div className="flex items-center gap-1.5 shrink-0">
              <span className="text-[10px] text-muted-foreground">优先级</span>
              <Slider min={0} max={100} step={5} value={[priority]} onValueChange={([v]) => setPriority(v)} className="w-20" />
              <span className="text-[10px] font-mono w-6 text-right">{priority}</span>
            </div>
          </div>
          <Textarea value={content} onChange={(e) => setContent(e.target.value)} className="min-h-[100px] text-[11px] resize-y leading-relaxed font-mono" />
          <div className="flex items-center justify-between">
            <Badge variant={atom.source === 'system' ? 'secondary' : atom.source === 'genre' ? 'default' : 'outline'} className="text-[9px] px-1.5 py-0">
              {atom.source === 'system' ? '系统' : atom.source === 'genre' ? '题材' : '用户'}
            </Badge>
            {changed && (
              <div className="flex gap-1.5">
                <Button variant="ghost" size="sm" className="h-6 text-[10px] px-2" onClick={() => { setContent(atom.content); setTitle(atom.title); setPriority(atom.priority); }}><RotateCcw className="h-3 w-3 mr-1" />撤销</Button>
...
function PlaybookTab({ bookId }: { bookId: string }) {
  ...
  const load = useCallback(() => { setLoading(true); getPromptTemplates(bookId).then(setTpl).finally(() => setLoading(false)); }, [bookId]);
  useEffect(() => { load(); }, [load]);
  ...
  const atoms = tpl?.ruleAtoms ?? [];
  const filtered = catFilter ? atoms.filter((a) => a.outputKey === catFilter) : atoms;
    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      <div className="mx-4 mt-3 rounded-md bg-amber-50/60 dark:bg-amber-950/20 border border-amber-200/50 px-3 py-2 flex items-start gap-2">
        ...
        <div className="text-[11px] text-muted-foreground leading-relaxed">
          <p>修改仅影响<strong>当前这本书</strong>。每条规则精确控制影响哪些 Agent。</p>
        </div>
      </div>
      <div className="flex flex-wrap gap-1 px-4 mt-2">
        <Button variant={!catFilter ? 'default' : 'outline'} size="sm" className="text-[10px] h-6 px-2" onClick={() => setCatFilter(null)}>全部 ({atoms.length})</Button>
        {ALL_CATEGORY_KEYS.map((key) => {
          const count = atoms.filter((a) => a.outputKey === key).length;
          const meta = CATEGORY_META[key];
          return (
            <Button key={key} variant={catFilter === key ? 'default' : 'outline'} size="sm" className="text-[10px] h-6 px-2" onClick={() => setCatFilter(catFilter === key ? null : key)}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-1.5">
        ...
        {sorted.map((atom) => <RuleAtomCard key={atom.id} atom={atom} bookId={bookId} onUpdated={load} />)}
        {!loading && sorted.length === 0 && <p className="text-xs text-muted-foreground text-center py-8">暂无规则</p>}
      </div>
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <div className="mx-4 mt-3 mb-2 rounded-md bg-amber-50/60 dark:bg-amber-950/20 border border-amber-200/50 px-3 py-2">
        <p className="text-[11px] text-muted-foreground leading-relaxed">
          每次修改规则或 Agent 指令时自动记录（最多保留 20 条）。点击「回滚」可将内容恢复到修改前的状态。
        </p>
      </div>
