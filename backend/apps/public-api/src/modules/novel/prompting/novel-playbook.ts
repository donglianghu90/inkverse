...
【七、旁观者升级——用围观者的反应衬托主角】
- 力量展示/重大事件时，按层级递进写旁观者反应：普通人震惊→强者侧目→大佬动容。
- 每一层的反应都要具体不同：有人杯子落地、有人不自觉后退一步、有人瞳孔微缩、有人站了起来。
- 旁观者的对话反应（"这……这怎么可能？""他什么时候……"）是读者获得满足感的重要通道。
...
export const CHAPTER_RHYTHM_PLAYBOOK = `章节节奏准则：

章内三幕结构（每章3000字内也要有起承转合）：
- 前20%（开头~600字）：承接上章+建立本章悬念/目标。开头第一段就要有"微钩子"——一句让人好奇的话、一个未解释的异常、一个冲突的苗头。
- 中间60%（~1800字）：推进并升级矛盾。每个段落至少包含一个动作推进、关系变化或信息增量。
- 后20%（结尾~600字）：章内小高潮+钩子。信息密度最高的部分。

呼吸节奏：
- 连续2-3段紧张段落后，需要一个"呼吸点"——一句幽默、一个微小的温情细节、一个安静的观察——然后猛然拉回紧张。
- 这个呼吸点让读者放松一瞬，下一波紧张感才能更强烈。
- 战斗/高潮段中穿插的一个不协调细节（"不知道谁的饭碗被震翻了"）反而能增强真实感。
...
export const FIRST_CHAPTERS_PLAYBOOK = `首章特殊约束（仅适用于前 3 章）：

黄金开局模板（第一章）：
- 第一段必须在100字内建立"不公/异常/危机"——让读者立刻产生情绪（愤怒/好奇/紧张）。
- 主角的第一个动作/选择必须让目标读者认同（"我也会这么做"或"我好想这么做"）。
- 前500字内必须暗示金手指/特殊能力的存在（不必展示，暗示即可）。
...
6) 每隔3-5章，至少有一个角色经历一次"内心考验"——选择、妥协、发现、醒悟。`;
...
结构（按顺序）：
1)【先抑——最后的压力】：敌人嘲讽达到顶峰、众人绝望、局势看似不可挽回。这段要写得让读者也觉得"完了"。
2)【转折信号】：一个微小但关键的变化——金手指激活、某人到来、主角眼神变化、一件旧物掉落。转折要小，但读者能感觉到风向变了。
3)【爆发序列】：力量展示。环境反应先于人物反应——天地变色→碎石腾空→气浪扩散→旁人震退。用短句、断句、动作密集。
4)【旁观者阶梯式震惊】：普通人呆住→内门弟子后退→长老站起来→宗主/大佬瞳孔微缩。每一层用不同的具体反应，不要重复"震惊"。
5)【金句收尾】：一句让读者想截图的话。简短、有力、有态度。
6)【安静的余韵】：所有人沉默。风吹过碎石。一个微小的声音（水滴声、旗帜飘动声）打破沉默。
...
铺垫章也要有吸引力：
- 即使在铺垫中，每500字至少有一个"微钩子"（一句反常的话、一个未解释的现象、一个角色的异常反应）。
