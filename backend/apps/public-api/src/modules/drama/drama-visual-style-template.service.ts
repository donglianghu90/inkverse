...
const SYSTEM_TEMPLATES: SystemVisualStyleTemplate[] = [
  /* ─── 真人影视 ─── */
  {
    ...
    visualGuide: {
      ...
      facePromptRule: '本剧为【现代都市真人】。\n\n【核心铁律】faceReferencePrompt 必须严格匹配角色的 gender（男/女）和职业身份，禁止用错性别。男性角色绝对禁止出现 woman/lady/female/goddess 等女性词汇；女性角色绝对禁止出现 businessman/gentleman 等纯男性词汇。每个角色的 faceReferencePrompt 必须各不相同，严禁多角色共用同一段描述。\n\n【格式要求】必须以 "modern contemporary" 或 "stylish urban" 开头，配合性别匹配的职业/身份词。\n示例（男性）："modern contemporary Chinese CEO, sharp confident face, strong jawline, front-facing, looking at camera, ..."\n示例（男性）："modern contemporary Chinese lawyer, intellectual refined features, clean-cut appearance, front-facing, looking at camera, ..."\n示例（女性）："modern contemporary Chinese businesswoman, elegant sharp features, confident gaze, front-facing, looking at camera, ..."\n示例（女性）："stylish urban Chinese designer, creative artistic aura, delicate features, front-facing, looking at camera, ..."\n\n必须包含 "front-facing, looking at camera"。真人风格质量要求：faceReferencePrompt 末尾必须加入 "eyes sharply in focus, clear iris detail, realistic skin texture with visible pores, natural subsurface scattering, no airbrushing, not plastic skin, 4K portrait masterpiece quality"，确保生成电影级人像质感而非磨皮商业照。',
    },
    promptGuidance: {
    ...
    genreCompatibility: ['都市', '职场', '悬疑', '爱情', '家庭', '霸总', '甜宠', '战神', '传记', '复仇', '逆袭'],
  ...
  },
  {
    ...
    visualGuide: {
      ...
      colorGrading: '暖金为主、层次丰富；柔和低饱和电影调色（非水墨），阴影暖琥珀、高光偏金；宫廷可略加深红棕，外景江湖可偏冷绿；整体避免国画/晕染式描述进英文 T2I',
      lightingStyle: '影视剧实拍光：伦勃朗侧光、宫廷烛光与窗光混合、外景柔和自然光与天光补光；不用「油画笔触/晕染」类词描述光',
      era: 'ancient',
      renderTechnique: '真实摄影，浅景深（f/2.0-f/4），精致服饰与建筑空间感，胶片颗粒',
      textureStyle: '丝绸刺绣、旧木与石材、金属反光等写实材质；配合 film grain，禁用 brush stroke / painterly',
      referenceStyle: '国产顶级古装剧《甄嬛传》《琅琊榜》《长安十二时辰》《鹤唳华亭》',
      styleReferencePrompt: 'cinematic Chinese period drama live-action photography, authentic ancient costume and set design, photorealistic, soft natural cinematic color grading warm highlights muted shadows, shallow depth of field, 35mm film grain texture, 4K ultra-detailed, premium broadcast period drama lighting, inspired by The Longest Day in Chang\'an cinematography, masterpiece',
      characterStylePrompt: 'Chinese historical drama, ancient period costume, cinematic hanfu, photorealistic portrait, film grain, silk and linen fabric texture, intricate embroidery detail, realistic skin with natural pores, no airbrushing, 4K portrait quality, neutral studio background, masterpiece period drama',
      facePromptRule: '本剧为【真人古装 / 中国历史题材实拍】（非国画水墨）。\n\n【核心铁律】faceReferencePrompt 必须严格匹配角色的 gender（男/女）和身份，禁止用错性别。男性角色绝对禁止出现 noblewoman/woman/lady/female/goddess 等女性词汇；女性角色绝对禁止出现 swordsman/emperor/warrior 等纯男性词汇。每个角色的 faceReferencePrompt 必须各不相同，严禁多角色共用同一段描述。\n\n【格式要求】必须以朝代词 + 性别匹配的身份词开头。\n示例（男性皇帝）："Tang dynasty ancient Chinese emperor in imperial dragon robe, commanding presence, front-facing, looking at camera, ..."\n示例（男性文人）："Tang dynasty ancient Chinese scholarly poet, refined elegant features, literary temperament, front-facing, looking at camera, ..."\n示例（男性官员）："Tang dynasty Chinese court official in purple robe, authoritative stern presence, front-facing, looking at camera, ..."\n示例（男性侠客）："ancient Chinese swordsman, sharp intense gaze, weathered rugged face, front-facing, looking at camera, ..."\n示例（男性太监）："Tang dynasty Chinese palace eunuch, smooth round face, shrewd cautious eyes, front-facing, looking at camera, ..."\n示例（女性贵族）："Tang dynasty ancient Chinese noblewoman in imperial court attire, graceful elegance, front-facing, looking at camera, ..."\n示例（女性道姑）："ancient Chinese Taoist priestess, ethereal serene face, delicate features, front-facing, looking at camera, ..."\n\n禁止仅写 "Chinese man/woman"，否则模型会生成现代人。必须包含 "front-facing, looking at camera"。定妆前缀已由 characterStylePrompt 提供，faceReferencePrompt 内禁止重复长段光影/调色。真人风格质量要求：faceReferencePrompt 末尾必须加入 "eyes sharply in focus, clear iris detail, realistic skin texture with visible pores, no airbrushing, not plastic skin, 4K portrait masterpiece quality, period-accurate makeup"，确保生成电影级人像质感。',
    },
    promptGuidance: {
      ...
      negativeKeywords: ['modern', 'contemporary', 'anime', 'fantasy CGI', 'neon light', 'casual clothing', 'low quality', 'worst quality', 'blurry', 'watermark', 'bad anatomy', 'plastic skin', 'deformed', 'ink wash', 'painterly', 'watercolor', 'illustration'],
      characterStyle: '精致汉服或宫廷服饰（纹样精细），传统发髻与头饰（层次丰富），古典妆容；服装须有织物纹理细节，皮肤质感真实，整体为实拍演员感而非绘画',
      backgroundStyle: '宫廷建筑（斗拱彩画金柱）、古典园林、江湖山水、茶楼客栈等古代场景；场景细节考究，建筑比例准确，无现代元素',
    },
    genreCompatibility: ['古装', '宫斗', '武侠', '历史', '穿越', '重生', '传记'],
  ...
  },
  {
    ...
    visualGuide: {
      ...
      facePromptRule: '本剧为【港片风格】。\n\n【核心铁律】faceReferencePrompt 必须严格匹配角色的 gender（男/女）和身份，禁止用错性别。男性角色绝对禁止出现 woman/lady/female 等女性词汇；女性角色绝对禁止出现 triad boss/gangster 等纯男性词汇（可用 female assassin/nightclub madam 等）。每个角色的 faceReferencePrompt 必须各不相同，严禁多角色共用同一段描述。\n\n【格式要求】必须以 "Hong Kong classic cinema style" 开头，配合性别匹配的身份词。\n示例（男性）："Hong Kong classic cinema style triad boss, sharp angular face, slicked-back hair, neon light on cheekbones, front-facing, looking at camera, ..."\n示例（男性）："Hong Kong classic cinema style undercover cop, weary determined face, stubble jaw, neon-lit side profile, front-facing, looking at camera, ..."\n示例（女性）："Hong Kong classic cinema style elegant nightclub singer, refined beautiful face, melancholy gaze, neon pink highlight on cheekbone, front-facing, looking at camera, ..."\n\n必须包含 "front-facing, looking at camera"。真人风格质量要求：faceReferencePrompt 末尾必须加入 "eyes sharply in focus, clear iris detail, 35mm film grain skin texture, neon color cast, realistic pores, no airbrushing, masterpiece quality"，确保生成胶片质感人像。',
    },
    promptGuidance: {
  ...
  /* ─── 真人实拍（补充） ─── */
  {
    ...
    visualGuide: {
      ...
      facePromptRule: '本剧为【复古武侠】。\n\n【核心铁律】faceReferencePrompt 必须严格匹配角色的 gender（男/女）和身份，禁止用错性别。男性角色绝对禁止出现 woman/lady/female 等女性词汇；女性角色绝对禁止出现 swordsman/warrior 等纯男性词汇（可用 swordswoman/female warrior/heroine）。每个角色的 faceReferencePrompt 必须各不相同，严禁多角色共用同一段描述。\n\n【格式要求】必须以 "retro wuxia film style" 开头，配合性别匹配的武侠身份词。\n示例（男性）："retro wuxia film style Chinese swordsman, weathered rugged face, intense gaze, front-facing, looking at camera, 1990s HK cinema aesthetic, ..."\n示例（男性）："retro wuxia film style martial arts master, aged wise face, deep-set piercing eyes, front-facing, looking at camera, ..."\n示例（女性）："retro wuxia film style Chinese swordswoman, sharp beautiful face, determined fierce gaze, wind-swept hair, front-facing, looking at camera, ..."\n\n必须包含 "front-facing, looking at camera"。真人风格质量要求：faceReferencePrompt 末尾必须加入 "eyes sharply in focus, clear iris detail, realistic skin texture with visible pores, weathered skin details, 35mm film grain quality, no airbrushing, masterpiece"，确保生成胶片质感人像。',
    },
    promptGuidance: {
  ...
  /* ─── 真人实拍（补充） ─── */
  {
    ...
    visualGuide: {
      ...
      facePromptRule: '本剧为【西方影视风格 / 好莱坞大片】。\n\n【核心铁律】faceReferencePrompt 必须严格匹配角色的 gender（男/女）和身份，禁止用错性别。男性角色绝对禁止出现 woman/lady/female 等女性词汇；女性角色绝对禁止出现 hero/gentleman 等纯男性词汇（可用 heroine/female agent）。每个角色的 faceReferencePrompt 必须各不相同，严禁多角色共用同一段描述。\n\n【格式要求】必须以 "cinematic Hollywood-style" 开头，配合性别匹配的身份词。\n示例（男性）："cinematic Hollywood-style American hero, chiseled weathered face, piercing eyes, dramatic lighting, front-facing, looking at camera, ..."\n示例（男性）："cinematic Hollywood-style veteran detective, rugged scarred face, tired but sharp eyes, front-facing, looking at camera, ..."\n示例（女性）："cinematic Hollywood-style female intelligence agent, sharp elegant features, cold calculating gaze, front-facing, looking at camera, ..."\n\n必须包含 "front-facing, looking at camera"。真人风格质量要求：faceReferencePrompt 末尾必须加入 "eyes sharply in focus, clear iris detail, realistic skin texture with visible pores, no airbrushing, not plastic skin, 4K portrait masterpiece quality"。',
    },
    promptGuidance: {
