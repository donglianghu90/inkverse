...
请严格审核以上内容，给出详细评分和问题列表。`,
  ...
  private buildShotDetail(storyboard: EpisodeStoryboard): string {
    ...
    // 优先取高潮/核心镜头（qualityTier=golden），再补充有台词的镜头，确保 ScriptReviewer 能看到最关键的视觉内容
    const midGolden = mid.filter((s: any) => s.qualityTier === 'golden').slice(0, 4).map((s: any) => this.formatShot(s));
    const midDialogue = mid.filter((s: any) => s.dialogue?.text && s.qualityTier !== 'golden').slice(0, 2).map((s: any) => this.formatShot(s));
    const midSample = [...midGolden, ...midDialogue];
    const last3 = shots.slice(-3).map((s: any) => this.formatShot(s));
    return [
      '【开场镜头】', ...first3,
      `【中段（${mid.length}个shot，抽样${midGolden.length}个高潮镜+${midDialogue.length}个有台词镜）】`, ...midSample,
      '【结尾镜头】', ...last3,
    ].join('\n');
  }
