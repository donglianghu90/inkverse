...
import type { LlmProvider, ModelTier, TaskRoute } from './llm.types';
...
const TASK_ROUTES: Record<string, TaskRoute> = {
  ...
  // ═══ OpenAI Creative (GPT-5) — 审阅/质量把关（跨模型审查，审查者能力须 ≥ 创作者） ═══
  'reader-jury':        { provider: 'openai', tier: 'creative' },
  'chapter-reviewer':   { provider: 'openai', tier: 'creative' },
  'editor-in-chief':    { provider: 'openai', tier: 'creative' },
  'consistency-audit':  { provider: 'openai', tier: 'creative' },
  'retrospective-learner': { provider: 'openai', tier: 'creative' },
  ...
  // ═══ Drama — 短剧引擎 ═══
  // --- 辅助/分析（Gemini Standard — 擅长分析推理，性价比高） ---
  ...
  // --- 叙事架构（GPT-5 Creative — 全剧结构是根基，需要最强叙事规划） ---
  'drama-series-director':    { provider: 'openai', tier: 'creative' },
  'drama-strategy':           { provider: 'openai', tier: 'creative' },
  'drama-arc-director':       { provider: 'openai', tier: 'creative' },
  'drama-episode-director':   { provider: 'openai', tier: 'creative' },
  // --- 核心创作（GPT-5 Creative — 编剧/对白/钩子是观众直接感知的内容） ---
  'drama-scriptwriter':       { provider: 'openai', tier: 'creative' },
  'drama-dialogue-coach':     { provider: 'openai', tier: 'creative' },
  'drama-script-editor':      { provider: 'openai', tier: 'creative' },
  'drama-hook-crafter':       { provider: 'openai', tier: 'creative' },
  'drama-storyboard-director': { provider: 'openai', tier: 'creative' },
  // --- 质量审查（GPT-5 Creative — 审查者能力须 ≥ 创作者） ---
  'drama-script-reviewer':    { provider: 'openai', tier: 'creative' },
  // --- 轻量校验/提取（Gemini Lightweight — 速度快成本低） ---
...
export class LlmService {
  ...
  private static parseTierCostRates(costObj: Record<string, unknown>, fallback: CostRates): Record<ModelTier, CostRates> {
    ...
    const globalIn = LlmService.num(costObj.inputPer1M, fallback.inputRatePer1M);
    const globalOut = LlmService.num(costObj.outputPer1M, fallback.outputRatePer1M);
    const result = {} as Record<ModelTier, CostRates>;
    for (const tier of tiers) {
      const tierObj = (costObj[tier] ?? {}) as Record<string, unknown>;
      result[tier] = {
        inputRatePer1M: LlmService.num(tierObj.inputPer1M, globalIn),
        outputRatePer1M: LlmService.num(tierObj.outputPer1M, globalOut),
      };
    }
    return result;
  }
  ...
  private buildAttemptChain(route: { provider: LlmProvider; tier: ModelTier; model: string }) {
    ...
    const fallbacks: LlmProvider[] = [
      this.cfg.fallbackProvider,
      ...(Object.keys(this.cfg.providers) as LlmProvider[]).filter(p => p !== route.provider && p !== this.cfg.fallbackProvider),
    ];
    for (const p of fallbacks) {
      if (p === route.provider) continue;
      const cfg = this.cfg.providers[p];
      if (!cfg?.enabled) continue;
      chain.push({ provider: p, tier: route.tier, model: cfg.models[route.tier] });
    }
    return chain;
  }
    ...
    for (let retry = 0; retry <= LlmService.CALL_MAX_RETRIES; retry++) {
      try {
        ...
        if (!retryable || retry >= LlmService.CALL_MAX_RETRIES) {
          ...
          const errContext = this.resolveUsageContext(errMeta, tags);
          this.usageLedger.record({
            userId: String(errMeta.userId ?? ''),
            module: errContext.module,
            resourceId: errContext.resourceId,
            scope: errContext.scope,
  ...
  private resolveUsageContext(
    ...
    tags: string[],
  ): { module: 'drama' | 'novel'; resourceId: string; scope: string } {
    const dramaId = typeof meta.dramaId === 'string' ? meta.dramaId : '';
    const bookId = typeof meta.bookId === 'string' ? meta.bookId : '';
    const episodeNumber = this.normalizeScopeNumber(meta.episodeNumber);
    const chapterNumber = this.normalizeScopeNumber(meta.chapterNumber);

    if (dramaId) {
      return {
        module: 'drama',
        resourceId: dramaId,
        scope: episodeNumber != null ? `episode:${episodeNumber}` : 'creation',
      };
    }
    if (bookId) {
      return {
        module: 'novel',
        resourceId: bookId,
        scope: chapterNumber != null ? `chapter:${chapterNumber}` : 'creation',
      };
    }

    const isDrama = tags.some(t => t.includes('drama'));
    return { module: isDrama ? 'drama' : 'novel', resourceId: '', scope: 'creation' };
  }

  private normalizeScopeNumber(value: unknown): number | null {
